...
# --- 1. CONFIGURAZIONE UI ---
st.set_page_config(page_title="AI SNIPER V11.39 - Expanded", layout="wide")

conn = st.connection("gsheets", type=GSheetsConnection)
API_KEY = '01f1c8f2a314814b17de03eeb6c53623'
TARGET_FINALE = 5000.0

# --- DIZIONARIO CAMPIONATI AGGIORNATO (Inghilterra e Olanda incluse) ---
LEAGUE_NAMES = {
...
# --- 3. INTERFACCIA ---
st.title("🎯 AI SNIPER V11.39 - Expanded")
...
with t1:
    ...
    soglia_v = st.slider("Soglia Valore %", 0, 15, 5) / 100

    if st.button("🚀 SCANSIONA", use_container_width=True):
        # Chiamata API usando la chiave selezionata
        ...
        if res.status_code == 200:
            ...
            st.sidebar.success(f"Crediti API: {res.headers.get('x-requests-remaining')}")
        else:
            st.error(f"Errore API: {res.status_code}. Controlla i crediti.")
            ...
            try:
                ...
                if opts:
                    ...
                    if val >= soglia_v:
                        ...
                        col_t.write(f"📅 {date_m} | **{nome_match}** | Valore: {round(val*100,1)}% (BK: {best['BK']})")
                        if col_b.button(f"ADD {best['Q']}", key=f"add_{nome_match}"):
                            # Stake fisso a 25€ come nella tua 11.39 o calcolato
...
with t3:
    st.subheader("📊 Storico & Goal")
    df_f = carica_db()
    if not df_f.empty:
        netto = round(df_f['Profitto'].sum(), 2)
        st.metric("Netto Totale", f"{netto} €", delta=f"{netto}€")
