...
# --- CONFIGURAZIONE UI ---
st.set_page_config(page_title="AI SNIPER V11.19 - Full Dashboard", layout="wide")
                ...
                if valid_options:
                    ...
                    if val_perc/100 > soglia_val:
                        ...
                        if col3.button("ADD", key=u_key):
                            nuova = {"Data Match": date_m, "Match": f"{home}-{away}", "Scelta": best['T'], "Quota": best['Q'], "Stake": stake, "Bookmaker": best['BK'], "Esito": "Pendente", "Profitto": 0.0, "Sport_Key": leagues[sel_league], "Risultato": "-"}
...
with t2:
    ...
    c1, c2 = st.columns(2)
    c1.metric("Esposizione Totale", f"{round(pendenti['Stake'].sum(), 2)} €")
    c2.metric("Ritorno Potenziale", f"{round((pendenti['Stake'] * pendenti['Quota']).sum(), 2)} €")
    st.write("---")
...
with t3:
    ...
    if not df_f.empty:
        ...
        st.dataframe(df_f[["Data Match", "Match", "Scelta", "Risultato", "Esito", "Profitto", "Bookmaker"]].sort_index(ascending=False))
        
        for i, row in df_f.iterrows():
            if row['Esito'] != "Pendente":
                status = "🟢" if row['Esito'] == "VINTO" else "🔴"
                st.write(f"{status} **{row['Match']}** | Ris: **{row['Risultato']}** | {row['Profitto']}€ ({row['Bookmaker']})")
